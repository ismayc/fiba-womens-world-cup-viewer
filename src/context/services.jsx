...
import { SERVICE_KEYS } from '../utils/watch.js'

const KEY = 'fwwc:services'
const VALID = new Set(SERVICE_KEYS)
const ServicesCtx = createContext(null)

// Inert fallback so components (and tests) render standalone without a provider,
// the same contract as the follow context.
const FALLBACK = {
  services: [],
  has: () => false,
  toggle: () => {},
...
// stored by catalog key. Persisted per-device in localStorage like a followed
// team, and deliberately NOT in the shareable URL: a link should not carry what
// someone pays for.
export function ServicesProvider({ children }) {
  const [services, setServices] = useState(() => {
    try {
      const saved = JSON.parse(localStorage.getItem(KEY) || '[]')
      // Drop keys the catalog no longer defines, so an old saved value cannot
      // linger and quietly filter the schedule against a service that is gone.
