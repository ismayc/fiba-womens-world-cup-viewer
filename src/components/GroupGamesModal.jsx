...
// Once a team has clinched a place in the final phase, show who they are set to
// play — the projected opponent (provisional until the bracket settles).
//
// A GROUP WINNER IS A SPECIAL CASE. It byes to a quarter-final, where its
// opponent is the winner of a qualification-round game that has not been played
// yet, so no group table can ever name it. Rather than showing an unqualified
// "To be determined", the pending feed label ("Winner Game 27") is shown, which
// is the true and more informative answer.
const ROUND_TITLE = { QR: 'Qualification round', QF: 'Quarter-final' }

function KnockoutSection({ team, knockout }) {
  if (!team || !knockout) return null
  const how =
    knockout.status === 'won-group'
      ? 'as group winners, with a bye straight to the quarter-finals'
      : knockout.status === 'second'
        ? 'in second place'
        : knockout.status === 'third'
          ? 'in third place'
      ...
      <div className="gg-ko-match">
        ...
        <span className="gg-ko-side">
          {knockout.opponent ? (
            ...
            <span className="gg-name gg-ko-tbd">
              {knockout.opponentLabel || 'To be determined'}
            </span>
          )}
        </span>
        {knockout.matchNum && <span className="gg-ko-num">Game {knockout.matchNum}</span>}
      </div>
      {!confirmed && (
        <p className="gg-ko-note">
          Projected matchup — the opponent can still change as the remaining games finish.
        </p>
      )}
    </div>
  )
...
// `knockout` is set only when a selected team has clinched a final-phase place.
