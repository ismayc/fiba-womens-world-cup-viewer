...
// score is provisional — decideGame already enforces that). Hidden in
...
export default function ChampionBanner({ match, hideScores }) {
  ...
  const champion = match ? decideGame(match)?.winner : null
  ...
  return (
    <button className="champ-banner" onClick={() => openDetail(match)} title="Open the Final">
      ...
      <span className="champ-text">
        👑 {FLAG_BY_TEAM[champion]} <strong>{champion}</strong> are the 2026 FIBA Women&rsquo;s World Cup champions! 🏆
      </span>
    </button>
  )
}
